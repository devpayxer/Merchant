-- 22 sep 2026 — DECISIÓN DEL DUEÑO: la competencia es el más barato, no la
-- mediana. "Un precio promedio no nos ayuda si hay alguien vendiendo más
-- barato; esa es nuestra competencia."
--
-- combo_stats gana (columnas AL FINAL, para poder usar OR REPLACE):
--   precio_min            el listado activo más barato (tras filtrar ruido)
--   precio_piso           el 3er listado activo más barato (limpio) = el
--                         competidor real. El mínimo pelado suele ser basura
--                         (otro carro, dañado) y el cuartil bajo se pasa de
--                         largo (BMW 3 Series: p25=$213 con competidores a
--                         $109). Si hay <3 limpios se usan todos; nunca
--                         supera a la mediana.
--   url_mas_barato / vendedor_mas_barato
--   precio_vendido        mediana del último precio de los listados que
--                         terminaron en 30 días (= a qué precio SE VENDE;
--                         se llena solo conforme el radar detecte vendidos)
-- Ruido filtrado: juegos/pares, llantas, TPMS, tapones, birlos; y títulos
-- que no mencionan ni la marca ni el modelo (rin de otro carro).
--
-- hot_list: precio_sugerido = piso × 0.95 ("entra justo debajo del
-- competidor real"); la ganancia se calcula sobre ese precio. La mediana
-- queda como precio_tipico (referencia de hasta dónde puede llegar).
-- Sustituye la regla anterior "10-15% bajo la mediana".
...
with base as (
  select c.id as combo_id, p.id as part_type_id, v.label as vehiculo,
         ...
         s.price,
         -- ruido: juegos/pares/llantas/tapones/birlos, o rin de otro carro
         (l.title is not null
...
select combo_id, part_type_id, vehiculo, make, model, year_start, year_end,
       ...
       last_checked_at,
       -- nuevas (al final). Si hay menos de 3 listados limpios, se usan todos.
       ...
       -- el link y el vendedor son los del PISO (3er más barato), es decir
       -- el competidor real, no el mínimo pelado que suele ser basura
...
select cs.vehiculo, cs.pieza, cs.vendidos_30d,
       round(cs.precio_mediano::numeric, 0) as precio_objetivo,   -- (= típico; se conserva por compatibilidad)
